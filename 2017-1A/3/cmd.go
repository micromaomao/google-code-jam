	...
	for currentDebuff <= maxDebuff {
		...
		for i := 0; i < tBA-1; {
			...
			if healthAfterThis <= 0 {
				// Maybe we shouldn't had attacked, but cured instead.
				if lastActionIsCure {
					impossible = true
					break
				} else {
		...
		if impossible {
			...
			if currentHealth-attack(Ak, currentDebuff+1, D) >= 0 {
				// We can debuff now and still survive
...
func optimizeBA(Hk, Ad, B int) int {
	if B == 0 {
		return int(math.Ceil(float64(Hk) / float64(Ad)))
	}
	...
	return b + int(math.Ceil(float64(Hk)/float64(Ad+B*b)))
}
